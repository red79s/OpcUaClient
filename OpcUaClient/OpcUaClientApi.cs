...
namespace OpcUaClient
{
    public class OpcUaClientApi
    {
        ...
        public Task<DataValue> ReadValueAsync(NodeId node)
        {
            return mSession.ReadValueAsync(node);
        }
        ...
        public async Task<StatusCode> WriteValueAsync(NodeId nodeId, object value, Type type = null)
        {
            var ct = new CancellationToken();

            Variant variant = new Variant(Convert.ChangeType(value, type ?? value.GetType()));
            ...
            var res = await mSession.WriteAsync(null, new WriteValueCollection(new List<WriteValue> { valueToWrite }), ct);
